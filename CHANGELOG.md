...
### Added
- WSL Integration for Windows: Sync SSH keys and configurations with Linux Subsystem
- New settings section for Windows users to select target WSL distribution
- Sync button in Settings to manually transfer keys to WSL home directory

## [1.1.2] - 2026-04-15

### Changed
- Replaced inline error messages with debounced toast notifications to prevent layout shifts
- Error toasts are now automatically dismissed as soon as the host name becomes unique
- Visual highlighting (red border/text) remains immediate for instant feedback
...
### Fixed
- SSH key installation to remote servers now works correctly when `sshpass` is not installed
- Added fallback to `expect` script for automated password entry during `ssh-copy-id`
- Password is now properly passed to the server during key installation in all cases

### Changed
- Improved error messaging with helpful hint to install `sshpass`: `brew install sshpass`
- Temporary expect script is automatically cleaned up after execution
